...
export default {
    props: {
        ...
        nextMonthCaption: {type: String, default: 'Siguiente mes'},
        prevMonthCaption: {type: String, default: 'Mes anterior'},
        setTimeCaption: {type: String, default: 'Agregar tiempo:'},
    ...
    computed: {
        ...
        currentPeriodDates() {
            ...
            days.forEach(day => {
                ...
                // day.dataActual = this.valueDate ? areSameDates(day.date, this.valueDate) : false
                if (this.selectedDates.length > 0 && !day.selected) {
                    for (let i = 0; i < this.selectedDates.length; i++) {
                        const _date = this.selectedDates[i];
                        if (areSameDates(day.date, this.parseDateString(_date, this.format))) {
                            day.selected = true;
                            break;
                        }
                    }
